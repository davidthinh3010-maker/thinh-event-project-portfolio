  ...
  return (
    <main>
      <nav className="nav">
        ...
        <div className="nav-links"><a href="#timeline">Timeline</a><a href="#about">About</a><a href="#contact">Contact</a></div>
      </nav>

      <section id="top" className="hero section-shell">
        <div className="eyebrow"><span className="dot"/> EVENT PROJECT PORTFOLIO · 2020—2026</div>
        <div className="hero-grid">
          <div><h1>EVENT<br/>PROJECT<br/><em>COORDINATOR</em></h1></div>
      ...
      <section id="about" className="intro section-shell">
        ...
        <div className="intro-grid">
          ...
          <div><p className="lead">Event and project coordination professional with 6+ years of experience across corporate events, exhibitions, retail projects, real estate launches, B2B forums and medical events.</p><div className="process">BRIEF <span>→</span> PLAN <span>→</span> COORDINATE <span>→</span> EXECUTE <span>→</span> HANDOVER <span>→</span> REPORT</div></div>
        </div>
      </section>

      <section id="timeline" className="timeline section-shell">
        <div className="section-label">02 / CAREER & PROJECT TIMELINE</div>
        <div className="timeline-head"><h2>2020 → 2026</h2><p>A project-led timeline showing what I worked on, where I contributed and how my role evolved. Projects with photography can be opened for more detail.</p></div>
            ...
            <article className="year-block" key={group.year}>
              ...
              <div className="year-content">
                ...
                {group.projects.map((project, index) => {
                  const canOpen = Boolean(project.overview || project.handled || project.image);
                  const isOpen = openProject === project.id;
                  return (
                    <div className={`project-wrap ${isOpen ? 'is-open' : ''}`} id={`project-${project.id}`} key={project.id}>
                      <button className={`project ${canOpen ? 'project-clickable' : ''}`} onClick={() => toggleProject(project)} aria-expanded={isOpen}>
                        <div className="project-num">{String(index + 1).padStart(2,'0')}</div>
                        <div className="project-main"><div className="project-meta"><span>{project.type}</span><span>{project.role}</span></div><h3>{project.name}</h3><p>{project.detail}</p></div>
                        {project.image ? <img className="project-thumb" src={project.image} alt={project.name} /> : <div className="project-placeholder">PROJECT</div>}
                        {canOpen ? (isOpen ? <X className="project-arrow" size={22}/> : <ArrowUpRight className="project-arrow" size={22}/>) : <span/>}
                      </button>
                      {isOpen && (
                        <div className="project-expanded">
                          {project.image && <img className="project-hero-image" src={project.image} alt={project.name} />}
                          <div className="project-detail">
                            <div><span className="detail-label">PROJECT OVERVIEW</span><p>{project.overview}</p></div>
                            <div><span className="detail-label">MY ROLE</span><p>{project.role}</p></div>
                            {project.handled && <div><span className="detail-label">WHAT I HANDLED</span><ul>{project.handled.map((item) => <li key={item}>{item}</li>)}</ul></div>}
                            {project.flow && <div className="flow-box"><span className="detail-label">PROJECT FLOW</span><p>{project.flow}</p></div>}
                          </div>
                        </div>
                      )}
                    </div>
      ...
      <section className="featured section-shell" id="case-studies">
        <div className="section-label">03 / FEATURED CASE STUDIES</div>
        <div className="featured-intro"><h2>SELECTED PROJECTS,<br/>BEYOND THE FINAL PHOTO.</h2><p>Each case highlights the project context, my role, the coordination work and the execution flow behind the result.</p></div>
        <div className="case-grid">
          {caseStudies.map((project, index) => (
            <article className="case-card" key={project.id}>
              <button onClick={() => openFromCase(project)} className="case-button">
                <div className="case-image-wrap"><img src={project.image} alt={project.name}/><span className="case-index">0{index + 1}</span></div>
                <div className="case-meta"><span>{project.type}</span><span>{project.role}</span></div>
                <h3>{project.name}</h3>
                <p>{project.overview}</p>
                <div className="case-link">VIEW PROJECT <ArrowUpRight size={18}/></div>
              </button>
            </article>
          ))}
        </div>
